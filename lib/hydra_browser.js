...
///**
// * Gets body with applied semantics, e.g. profile semantics or linked data semantics.
// * @returns {*}
// */
//HydraBrowser.prototype.interpretedBody = function () {
//  return this.compactedBody;
//};
...
HydraBrowser.prototype.parse = function (response, baseUrl) {
  ...
  return response.json()
    .then(function (json) {
      ...
      return jsonld.promises.compact(json, {}, {base: baseUrl})
        .then(function (compactedBody) {
          self.compactedBody = compactedBody;
          return response;
        });
    });
};
...
HydraBrowser.prototype.findAffordance = function (rel, currentUrl) {
  ...
  return new Promise(function (resolve) {
    resolve(self.affordanceFinder.findAffordance(self.compactedBody, rel, currentUrl));
  });
};
    ...
    for (i = 0; i < len; i++) {
      ...
      for (rel in linkRelMatchers) {
        if (linkRelMatchers.hasOwnProperty(rel)) {
          ...
          var link = parsedLinkHeader.rels[rel]; // TODO: might not be the expected link
